...
function alterarQuantidade() {
  listarItens();
  const index =
    parseInt(prompt("Digite o número do item que deseja alterar:"), 10) - 1;
  ...
  if (estoque[index]) {
    ...
    if (!isNaN(novaQuantidade)) {
      ...
      alert(
        `Quantidade do item "${estoque[index].nome}" alterada para ${novaQuantidade}.`
      );
    } else {
      alert("Quantidade inválida.");
    }
...
function excluirItem() {
  listarItens();
  const index =
    parseInt(prompt("Digite o número do item que deseja excluir:"), 10) - 1;
